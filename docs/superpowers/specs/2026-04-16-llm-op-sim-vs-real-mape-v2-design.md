...
| | | | | | **最终归类（2026-04-17 修订）**: **简单（trace 阶段未启用 DYNAMIC_KERNEL_RANGE 过滤 + kernelslist.g 后处理也未剔除 PyTorch init kernel）** — kernel-198 是 PyTorch `DistributionNormal` (curand 正态分布初始化, ~22万线程) 而非 attention 路径上的 kernel；log 末尾 `Destroy streams for kernel 198: size 0` 是 GPGPU-Sim kernel 完成的正常打印，**kernel-198 实际成功结束**，sim 在其后被外部 timeout/OOM kill。**v2 fix**：trace 阶段强制启用 NVBit `DYNAMIC_KERNEL_RANGE` 按 kernel 名 regex 过滤，从源头剔除 PyTorch 数据 init kernel；详见 plan Phase 2 Task 2.1。 |
