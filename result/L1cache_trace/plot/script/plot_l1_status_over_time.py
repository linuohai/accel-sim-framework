...
L1_TRACE_DIR = RESULT_DIR / "L1cache_trace"
DEFAULT_OUT_DIR = PLOT_DIR / "result" / "l1_cache"
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=Path,
        help="Input CSV path(s). If omitted, auto-pick from result/L1cache_trace.",
    )
    parser.add_argument(
        "--csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Pick input CSV by stem from result/L1cache_trace (can be repeated). "
            "Example: bfs_web_l1"
        ),
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Plot all matching CSVs under result/L1cache_trace.",
    )
    parser.add_argument(
        "--points",
        type=int,
...
def resolve_inputs(args: argparse.Namespace) -> List[Path]:
    ...
    if args.stem:
        for stem in args.stem:
            ...
            if not stem:
                continue
            path = (L1_TRACE_DIR / f"{stem}.csv").resolve()
            inputs.append(path)

    if args.all:
        inputs.extend(_discover_candidates(L1_TRACE_DIR))

    if not inputs:
        inputs = [discover_default_input(L1_TRACE_DIR)]
