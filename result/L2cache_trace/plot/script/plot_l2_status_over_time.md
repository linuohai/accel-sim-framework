# L1/L2 Cache 状态随时间变化（plot_l2_status_over_time.py）
...
- `--stem <name>`：从 `result/L2cache_trace/` 里按 stem 选择（可重复），例如 `--stem bfs_web_l2`
- `--all`：把 `result/L2cache_trace/` 下所有 CSV 都画一遍

如果以上都不提供，脚本会从 `result/L2cache_trace/` 中自动挑选“最新且包含必要列”的 CSV。
...
python3 plot_l2_status_over_time.py --normalize --format svg
```
