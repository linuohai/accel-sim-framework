#!/usr/bin/env python3
"""Round-2 Quality Fig (b): Data Coverage — full 27 workloads (MST excluded).

Single-row, full-width (7.0 x 1.8) bar chart of GRASP Data Coverage (%):

    data_coverage = (baseline.ima_data_misses - grasp.ima_data_misses)
                    / baseline.ima_data_misses * 100
...
def compute_data_coverage(key: str):
    """Return data coverage % or None if baseline IMA data missing."""
    rec = DATA.get(key)
    if rec is None:
        return None
    base = rec.get("baseline") or {}
    g = rec.get("grasp") or {}
    bm = base.get("ima_data_misses")
    gm = g.get("ima_data_misses")
    if bm is None or gm is None or bm == 0:
        return None
    return (bm - gm) / bm * 100.0


values: list[float | None] = [compute_data_coverage(k) for k in ORDERED_27]
...
ax.set_ylabel("Data Coverage (%)", fontsize=7)
ax.set_ylim(-10, 100)
...
ax.set_title(
    f"(b) Data Coverage   (avg = {avg_value:.1f}%)",
    fontsize=8, pad=3,
)

save_fig(fig, "proto_r2_q2_data_coverage", OUT_DIR)
plt.close(fig)

print(f"Saved: {OUT_DIR}/proto_r2_q2_data_coverage.{{pdf,svg}}")
