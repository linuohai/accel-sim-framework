#!/usr/bin/env python3
"""
Round 5 §5.4 Layout B — Throttle Accuracy (nohatch, full-width bar).

15 workloads × 3 schemes: Default, D5b, T40C200.
Y = Accuracy % (already a percentage, not a multiplier).
...
from _data import THROTTLE_ACC  # noqa: E402
...
WORKLOADS = [k for k in ORDERED_27 if k in THROTTLE_ACC]
n_wkl = len(WORKLOADS)
assert n_wkl == 15, f"Expected 15 accuracy workloads, got {n_wkl}"


def col_arr(idx):
    return np.array([THROTTLE_ACC[k][idx] for k in WORKLOADS])
...
default_mean = float(np.mean(default_a))
d5b_mean     = float(np.mean(d5b_a))
t40_mean     = float(np.mean(t40_a))

default_all = np.append(default_a, default_mean)
d5b_all     = np.append(d5b_a,     d5b_mean)
t40_all     = np.append(t40_a,     t40_mean)
...
schemes = [
    ...
    ("D5b",     d5b_all,     PALETTE_TOL_BRIGHT[0]),  # green
...
for gname, gwkls in ALG_GROUPS:
    present = [w for w in gwkls if w in THROTTLE_ACC]
    if not present:
        continue
...
subplot_label(ax, "(c) Throttle Accuracy", y=-0.55)


out_dir = Path(__file__).resolve().parent
save_fig(fig, "proto_r4_54_B_acc", out_dir)
plt.close(fig)


print("=== Round 5 §5.4 Throttle Accuracy (nohatch) ===")
print(f"Workloads: {n_wkl}  |  MEAN per scheme:")
print(f"  Default  {default_mean:.2f}%")
print(f"  D5b      {d5b_mean:.2f}%")
print(f"  T40C200  {t40_mean:.2f}%")
print(f"\nSaved to {out_dir}/proto_r4_54_B_acc.{{pdf,svg}}")
