#!/usr/bin/env python3
"""
Round 5 §5.4 Combined alternative — Distance + Storage sensitivity (nohatch).

1×2 horizontal subplots (full-width, 7.0 x 2.4):
  (a) Distance sensitivity — x=D{1,2,4,8}, y=normalized speedup over D=1
  (b) Storage sensitivity  — x=S{1..4},    y=normalized speedup over S1

Each panel has 6 per-algorithm GMEAN lines + bold overall GMEAN.
"""
from __future__ import annotations
import sys
from pathlib import Path
import numpy as np

_REPO = Path(__file__).resolve().parents[5]
sys.path.insert(0, str(_REPO / "ima_plan/07_paper_outline/plot_style"))
sys.path.insert(0, str(Path(__file__).resolve().parent))

from plot_util import (  # noqa: E402
    apply_style, save_fig,
    PALETTE_TOL_BRIGHT, subplot_label,
)
from _data import (  # noqa: E402
    DISTANCE_IPC, STORAGE_IPC, DISTANCES, STORAGE_LABELS, SENS_ALGORITHMS,
    sensitivity_normalized, algorithm_gmean_lines, overall_gmean_line,
)

apply_style()
import matplotlib.pyplot as plt  # noqa: E402
from matplotlib.lines import Line2D  # noqa: E402


# ── Distance data ──────────────────────────────────────────────────────────
dist_wkls = list(DISTANCE_IPC.keys())
dist_norm = sensitivity_normalized(DISTANCE_IPC, dist_wkls)
dist_alg_lines = algorithm_gmean_lines(dist_norm, SENS_ALGORITHMS)
dist_overall = overall_gmean_line(dist_norm)

# ── Storage data ───────────────────────────────────────────────────────────
stor_wkls = list(STORAGE_IPC.keys())
stor_norm = sensitivity_normalized(STORAGE_IPC, stor_wkls)
stor_alg_lines = algorithm_gmean_lines(stor_norm, SENS_ALGORITHMS)
stor_overall = overall_gmean_line(stor_norm)


ALG_COLORS = {
    "BFS":  PALETTE_TOL_BRIGHT[0],
    "SSSP": PALETTE_TOL_BRIGHT[1],
    "BC":   PALETTE_TOL_BRIGHT[2],
    "CC":   PALETTE_TOL_BRIGHT[3],
    "SpMV": PALETTE_TOL_BRIGHT[4],
    "VC":   "#4B4B6A",
}
ALG_MARKERS = {
    "BFS":  "o",
    "SSSP": "s",
    "BC":   "^",
    "CC":   "D",
    "SpMV": "v",
    "VC":   "*",
}


fig, axes = plt.subplots(
    nrows=1, ncols=2,
    figsize=(7.0, 2.5),
    constrained_layout=True,
)


def plot_panel(ax, alg_lines, overall, x, xticklabels, xlabel, ylabel,
               ylo, yhi, yticks, subtitle, ref_y=1.0,
               clip_upper=None):
    for name, line, _ in alg_lines:
        # For clipping — use clipped values but annotate real ones
        y_plot = np.minimum(line, clip_upper) if clip_upper else line
        ax.plot(
            x, y_plot,
            marker=ALG_MARKERS[name], color=ALG_COLORS[name],
            markersize=5.0, linewidth=1.2, alpha=0.75,
            markeredgecolor="white", markeredgewidth=0.35,
            zorder=5,
        )
        if clip_upper:
            for j, v in enumerate(line):
                if v > clip_upper:
                    ax.annotate(
                        f"{v:.2f}\u00d7",
                        xy=(x[j], clip_upper),
                        xytext=(3, -1), textcoords="offset points",
                        ha="left", va="top",
                        fontsize=5.0, fontweight="bold",
                        color=ALG_COLORS[name],
                        zorder=11,
                    )
    ax.plot(
        x, overall,
        marker="o", color="black",
        markersize=6.0, linewidth=2.2,
        markeredgecolor="white", markeredgewidth=0.6,
        zorder=10,
    )
    ax.axhline(ref_y, color="#888888", linewidth=0.6,
               linestyle=":", alpha=0.7, zorder=1)
    ax.set_xticks(x)
    ax.set_xticklabels(xticklabels, fontsize=7)
    ax.set_xlim(-0.3, len(x) - 1 + 0.3)
    ax.set_ylim(ylo, yhi)
    ax.set_yticks(yticks)
    ax.set_yticklabels([f"{t:.2f}\u00d7" for t in yticks])
    ax.tick_params(axis="y", labelsize=7)
    ax.set_xlabel(xlabel, fontsize=8, labelpad=2)
    ax.set_ylabel(ylabel, fontsize=8, labelpad=2)
    subplot_label(ax, subtitle, y=-0.38)


# (a) Distance
x_d = np.arange(len(DISTANCES))
plot_panel(
    axes[0], dist_alg_lines, dist_overall,
    x=x_d,
    xticklabels=[f"D={d}" for d in DISTANCES],
    xlabel="Prefetch Distance",
    ylabel="Normalized Speedup over D=1",
    ylo=0.95, yhi=1.10,
    yticks=[0.95, 0.97, 1.00, 1.03, 1.06, 1.10],
    subtitle="(a) Distance Sensitivity",
    clip_upper=1.10,
)

# (b) Storage
x_s = np.arange(len(STORAGE_LABELS))
plot_panel(
    axes[1], stor_alg_lines, stor_overall,
    x=x_s,
    xticklabels=STORAGE_LABELS,
    xlabel="Per-SM Storage Budget",
    ylabel="Normalized Speedup over S1",
    ylo=0.99, yhi=1.04,
    yticks=[0.99, 1.00, 1.01, 1.02, 1.03, 1.04],
    subtitle="(b) Storage Sensitivity",
    clip_upper=1.04,
)
# Flat-data note on storage panel
axes[1].text(
    0.02, 0.96,
    "16/20 wkls flat",
    transform=axes[1].transAxes,
    ha="left", va="top",
    fontsize=5.5, color="#444444",
    fontstyle="italic",
)


# ── Legend shared above figure ────────────────────────────────────────────
handles = []
for name, _, n_wl in dist_alg_lines:
    handles.append(Line2D(
        [0], [0],
        marker=ALG_MARKERS[name], color=ALG_COLORS[name],
        markersize=5, linewidth=1.2,
        markeredgecolor="white", markeredgewidth=0.35,
        label=f"{name}",
    ))
handles.append(Line2D(
    [0], [0],
    marker="o", color="black",
    markersize=5.5, linewidth=2.0,
    markeredgecolor="white", markeredgewidth=0.5,
    label="Overall GMEAN",
))

fig.legend(
    handles=handles, ncol=7, fontsize=6.5,
    loc="lower center", bbox_to_anchor=(0.5, 1.00), frameon=False,
    handlelength=1.6, handleheight=0.9,
    columnspacing=1.2, handletextpad=0.3,
    borderaxespad=0,
)


out_dir = Path(__file__).resolve().parent
save_fig(fig, "proto_r4_54_CD_sens", out_dir)
plt.close(fig)


print("=== Round 5 §5.4 Combined Sensitivity (nohatch) ===")
print(f"Panel (a) Distance overall: " +
      " ".join(f"{v:.3f}" for v in dist_overall))
print(f"Panel (b) Storage  overall: " +
      " ".join(f"{v:.4f}" for v in stor_overall))
print(f"\nSaved to {out_dir}/proto_r4_54_CD_sens.{{pdf,svg}}")
