#!/usr/bin/env python3
"""
Round 4 §5.3 FINAL — Accuracy figure (GRASP vs SOTA prefetch accuracy).
...
GRASP accuracy:
    ...
    (this is the definition already stored in grasp.grasp_effect.accuracy_pct;
    late prefetches are treated as neutral — not useless.)

SOTA accuracy values are parsed from
...
Output: proto_r4_53_accuracy.{pdf,svg}
"""
...
# ── GRASP accuracy (existing computation) ───────────────────────────────────
def grasp_accuracy(g: dict) -> float:
    ge = g.get("grasp_effect") or {}
    ap = ge.get("accuracy_pct")
    if ap is None:
        # Re-derive from raw counts if the field is missing.
...
grasp_acc = np.array([grasp_accuracy(DATA[k]["grasp"])
                      for k in ORDERED_27], dtype=float)
snake_acc = np.array([SOTA.get(k, {}).get("Snake",     {}).get("accuracy", 0.0)
                      for k in ORDERED_27], dtype=float)
caps_acc  = np.array([SOTA.get(k, {}).get("CAPS",      {}).get("accuracy", 0.0)
                      for k in ORDERED_27], dtype=float)
spare_acc = np.array([SOTA.get(k, {}).get("Spare Reg", {}).get("accuracy", 0.0)
                      for k in ORDERED_27], dtype=float)

grasp_mean = float(np.mean(grasp_acc))
snake_mean = float(np.mean(snake_acc))
caps_mean  = float(np.mean(caps_acc))
spare_mean = float(np.mean(spare_acc))
...
# ── Append AVG column ───────────────────────────────────────────────────────
grasp_all = np.append(grasp_acc, grasp_mean)
snake_all = np.append(snake_acc, snake_mean)
caps_all  = np.append(caps_acc,  caps_mean)
spare_all = np.append(spare_acc, spare_mean)
...
# ── Bottom subtitle ─────────────────────────────────────────────────────────
subplot_label(ax, "(c) Prefetch Accuracy", y=-0.55)


# ── Save ────────────────────────────────────────────────────────────────────
out_dir = Path(__file__).resolve().parent
save_fig(fig, "proto_r4_53_accuracy", out_dir)
plt.close(fig)


# ── Console summary ─────────────────────────────────────────────────────────
print("=== Round 4 §5.3 Accuracy — GRASP vs SOTA (4 bars) ===")
print(f"workloads = {n_wkl}")
print(f"  GRASP     avg = {grasp_mean:5.2f}%  "
      f"min = {float(np.min(grasp_acc)):5.2f}%  "
      f"max = {float(np.max(grasp_acc)):5.2f}%  "
      f"median = {float(np.median(grasp_acc)):5.2f}%")
print(f"  Snake     avg = {snake_mean:5.2f}%  "
      f"min = {float(np.min(snake_acc)):5.2f}%  "
      f"max = {float(np.max(snake_acc)):5.2f}%")
print(f"  CAPS      avg = {caps_mean:5.2f}%  "
      f"min = {float(np.min(caps_acc)):5.2f}%  "
      f"max = {float(np.max(caps_acc)):5.2f}%")
print(f"  Spare Reg avg = {spare_mean:5.2f}%  "
      f"min = {float(np.min(spare_acc)):5.2f}%  "
      f"max = {float(np.max(spare_acc)):5.2f}%")
...
print(f"\nSaved to {out_dir}/proto_r4_53_accuracy.{{pdf,svg}}")
