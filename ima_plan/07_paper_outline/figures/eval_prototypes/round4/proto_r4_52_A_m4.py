#!/usr/bin/env python3
"""
Round 4 §5.2 Layout A — M4 VARIANT (Magellan-inverse hatching).

Sibling of proto_r4_52_A.py (Option B = shapes). Renders the same data
and layout but with Magellan-inspired hatching: SOTA baselines carry all
the texture, and GRASP is solid (no hatch) — matching the ISCA'25
Magellan convention that "our work = solid, visually calmest".

M4 hatching assignment:
  - Snake     → "/////" dense diagonal           (like APT-GET in Magellan Fig 15)
  - CAPS      → "xxxxx" dense crosshatch         (density contrast vs Snake)
  - Spare Reg → "\\\\\\\\\\" dense back-diagonal (mirror of Snake)
  - GRASP     → ""      solid, no hatch          (our-work convention)
...
Output: proto_r4_52_A_m4.{pdf,svg} in this directory.
"""
...
#   GRASP     → dots          "....."
schemes = [
    ("Snake",     snake_all, PALETTE_TOL_BRIGHT[1], "/////"),    # red,    diagonal
    ("CAPS",      caps_all,  PALETTE_TOL_BRIGHT[2], "xxxxx"),    # purple, crosshatch
    ("Spare Reg", spare_all, PALETTE_TOL_BRIGHT[3], "\\\\\\\\\\"),  # yellow, back-diag
    ("GRASP",     grasp_all, PALETTE_TOL_BRIGHT[0], ""),         # green,  solid
]
...
save_fig(fig, "proto_r4_52_A_m4", out_dir)
plt.close(fig)


# ── Console summary ─────────────────────────────────────────────────────────
print("=== Round 4 §5.2 Layout A — M4 (Magellan-inverse) ===")
...
print(f"\nSaved to {out_dir}/proto_r4_52_A_m4.{{pdf,svg}}")
