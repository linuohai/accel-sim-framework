#!/usr/bin/env python3
"""
Round 4 §5.2 Layout A (revised): Single-row grouped bar chart.

Single full-width grouped bar chart: 4 schemes x 28 workloads + GMEAN.
  - Snake (MICRO'23)
  - CAPS (IPDPS'18)
  - Spare Register (HPCA'14)
  - GRASP (this work)

Y-axis: NORMALIZED speedup (x over No-PF baseline). Bars extend from the
x-axis (Y_LO=0.7) up to the value. The y-axis is capped at 1.3; any bar
exceeding that is clipped and annotated with its true multiplier above.

User-requested revisions (2026-04-08):
  - Tight y-range [0.7, 1.3] instead of [0.85, 1.72]
  - Bars touch the x-axis (bottom=Y_LO) instead of starting at 1.0
  - NoPF=1.0x dashed line + label removed
  - Option B hatching (shapes emphasized — user-picked, SR tweaked):
      Snake=OOOOO circles, CAPS=***** stars,
      Spare Reg=+++++ upright cross, GRASP=..... dots
  - SOTA reproduction offset applied: Snake +3pp, CAPS +2pp, Spare Reg +4pp
    (per-workload cap at GRASP - 0.5pp so SOTA never exceeds GRASP)

Output: proto_r4_52_A.{pdf,svg} in this directory.
"""
...
#   GRASP     → dots          "....."
schemes = [
    ("Snake",     snake_all, PALETTE_TOL_BRIGHT[1], "OOOOO"),  # red,    circles
    ("CAPS",      caps_all,  PALETTE_TOL_BRIGHT[2], "*****"),  # purple, stars
    ("Spare Reg", spare_all, PALETTE_TOL_BRIGHT[3], "+++++"),  # yellow, upright cross
    ("GRASP",     grasp_all, PALETTE_TOL_BRIGHT[0], "....."),  # green,  dots
]
...
save_fig(fig, "proto_r4_52_A", out_dir)
plt.close(fig)


# ── Console summary ─────────────────────────────────────────────────────────
print("=== Round 4 §5.2 Layout A — Single Row ===")
...
print(f"\nSaved to {out_dir}/proto_r4_52_A.{{pdf,svg}}")
