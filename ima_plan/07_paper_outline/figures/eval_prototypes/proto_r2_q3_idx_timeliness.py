#!/usr/bin/env python3
"""Round-2 Quality Fig (c): Index Timeliness — full 27 workloads (MST excluded).

Single-row, full-width (7.0 x 1.8) bar chart of GRASP Index Timeliness (%).
Source field: ``grasp.ima_timeliness.index_pct`` from
``effectiveness_data.json`` (extracted from the GRASP_TIMELINESS log line).
"""
...
def get_idx_timeliness(key: str):
    rec = DATA.get(key)
    if rec is None:
        return None
    g = rec.get("grasp") or {}
...
values: list[float | None] = [get_idx_timeliness(k) for k in ORDERED_27]
...
ax.set_ylabel("Index Timeliness (%)", fontsize=7)
ax.set_ylim(0, 110)
...
ax.set_title(
    f"(c) Index Timeliness   (avg = {avg_value:.1f}%)",
    fontsize=8, pad=3,
)

save_fig(fig, "proto_r2_q3_idx_timeliness", OUT_DIR)
plt.close(fig)

print(f"Saved: {OUT_DIR}/proto_r2_q3_idx_timeliness.{{pdf,svg}}")
