...
DO NOT render any metadata, section headers, block labels, or layout percentages. Do NOT write "BLOCK A", "BLOCK B", "BLOCK C", "TOP SECTION", "BOTTOM SECTION", "FORMULA BAR" or any similar structural labels as visible text in the figure.

=== LAYOUT ===
Upper 2/3: three blocks (two stacked left, one tall right). Left+right tops and bottoms aligned.
Lower 1/3: one wide CT table (left ~75%) + one IST block (right ~25%).
...
Data row: GREEN #5AA469 square | c[0x0][0x180] (&x[]) | 4 (sizeof float) |
...
Monospace code, ONLY lines marked (1) or (2) are colored. ALL other code lines are GRAY:
...
=== LOWER LEFT (~75% width): Chain Table (CT) ===
...
=== LOWER RIGHT (~25% width): IST Block ===
Title: "Iteration Stride Tracker (IST)" bold, placed above the block.
Blue #2B5EA7 outlined box with light background, same height as CT table.

Content showing per-PC stride learning mechanism:
  PC 0x03d0, Warp W:
    1st visit: addr = A1
    2nd visit: addr = A2
    stride = A2 - A1 = 64 B
Small italic text at bottom: "Any warp can contribute"

Arrow from IST pointing LEFT to CT "Iter Stride" column, showing that IST feeds the stride value into CT.

=== ARROWS (CORRECT DIRECTIONS) ===
1. Gray dashed: SpMV CUDA -> SASS ("compiles to")
2. SASS to TT: Gray dashed arrows from IMAD.WIDE operands "c[0x180]" and "R16" pointing LEFT to TT Base Address and Scale cells
3. SASS to CT: Black solid arrows from SASS index load lines (/*03d0*/ and /*03e0*/) pointing DOWN to corresponding CT rows, labeled (1) and (2). Direction: FROM SASS TO CT.
...
6. IST to CT: Arrow from IST block LEFT to CT stride column

=== ANNOTATIONS (bold text, NOT dialog boxes) ===
Just bold colored text placed in blank areas with thin pointing lines. NO background boxes. NO gray rounded boxes. NO borders around annotations.
